...
authModule
.controller('authController', function($scope,$rootScope,authService,AUTH_EVENTS,$uibModal) {
	...
	var callbackSuccess = function(data,headers) { // Status Code:200A")
        if(data.success) {
    ...
    authCtrl.openPop = function(msgToDisplay) {
...
authModule.factory('authService', function($rootScope,$http,$timeout,$cookieStore,$window,APP_CONSTANT,AUTH_EVENTS,Base64) {
		var authService = {};
        ...
        // var authdata = data.name + ':' + data.id;
         ...
         var authKey= APP_CONSTANT.DEMO?'dummy':headers(APP_CONSTANT.AUTH_KEY);
...
authModule.factory('Base64', function () {
    /* jshint ignore:start */
